...
A fold that reads a whole as its parts together (`Cl` with `DCl` and `VCl`) is built but
not offered in the page yet; a link carrying `F` among its `v=` flags turns it on. With it
on, five more columns follow everything else — `folded_label_AP_bregma_mm`,
`folded_label_ML_abs_mm`, `folded_label_DV_mm`, `folded_n_labels`, `folded_parts` — filled
for the nine such wholes and blank on every other row. The columns before them are read off
the atlas's own labels whether or not the fold is on, so they are the same bytes either way.
The Labels CSV never carries the fold: a row of it is a printed label, and the plate prints
`DCl`.
