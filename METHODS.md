...
test reads them unchanged. **<!-- n:region_extents.summary.structure_plate_entries -->3,134<!-- /n --> structure–plate entries carry an area**, 97% of the 3,216 the
label pass located and 93% of the 3,369 the published index lists (both counted over the
structures that are regions), as <!-- n:region_extents.summary.polygons -->5,952<!-- /n --> polygons over <!-- n:region_extents.summary.points -->156,004<!-- /n --> points.
...
4. **Cut the empty space into faces.** A face sealed by traced ink and holding exactly one
   abbreviation is that structure's area *as drawn*: <!-- n:region_extents.summary.faces_named_by_one_abbreviation -->3,544<!-- /n --> faces. A label the atlas set
   outside its region is seeded at the end of the line it draws rather than on the word
   (<!-- n:region_extents.summary.labels_on_a_leader -->200<!-- /n --> labels; see [Where the name is not the place](#where-the-name-is-not-the-place)).
5. **Letter the hemisphere the atlas did not.** Some abbreviations are set once (`S1J` on
   ...
   a boundary the other does not. <!-- n:region_extents.summary.labels_mirrored -->120<!-- /n --> seeds are added this way; they carry no weight in
...
| Printed labels inside the region they name | **<!-- n:region_extents.summary.label_inside_its_own_region -->0.977<!-- /n -->**, read at the end of the label's line where the atlas draws one |
...
median <!-- n:region_extents.summary.traced_fraction_median -->0.999<!-- /n -->, a share of <!-- n:region_extents.summary.traced_fraction_ge_90 -->0.796<!-- /n --> at or above 0.90, <!-- n:region_extents.summary.traced_fraction_ge_75 -->0.89<!-- /n --> at or above 0.75,
...
240 of the printed labels carry a line the leader pass read, and <!-- n:region_extents.summary.labels_on_a_leader -->200<!-- /n --> are seeded at
...
names). A further <!-- n:region_extents.summary.labels_relocated -->222<!-- /n --> sit outside the face they name with no line to follow, printed on
...
the surface. It is left off, and the surface contains **<!-- n:vol:checks.labels_inside_the_surface -->0.9691<!-- /n -->** of the printed labels,
...
| --- | --- | --- |
| Cross-section area on a plate a structure was built from, against `region_extents` | — | **median <!-- n:vol:checks.section_area_median_rel_error -->0.0112<!-- /n --> off**, 90th percentile <!-- n:vol:checks.section_area_p90_rel_error -->0.0532<!-- /n -->, the lattice's own quantisation |
| Regions partition the volume | a point is inside one region or none | **holds**: every voxel inside the surface carries exactly one label or is an unnamed sealed face, <!-- n:vol:checks.unnamed_fraction -->0.0194<!-- /n --> of the brain |
...
| Printed labels inside the region they name | <!-- n:region_extents.summary.label_inside_its_own_region -->0.977<!-- /n --> | **<!-- n:vol:checks.labels_in_their_own_region -->0.9587<!-- /n -->**: the 2-D figure is after labels were pulled to the nearest face; this one is not |
...
voxel inside the voxels it was cut from accounts for another <!-- n:vol:checks.mesh_vs_voxel_median_rel_error -->0.0496<!-- /n -->, the difference
...
slits, `DCl`) a median <!-- n:vol:checks.fragment_share_when_more_median -->0.1439<!-- /n --> of it sits outside the largest two. This is reported
