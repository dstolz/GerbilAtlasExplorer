...
/* the regions Ctrl/Cmd-clicked in beside the selection. The card describes `sel` alone;
   these are outlined on the plate with it, and are never empty while `sel` is null */
const msel = new Set();
...
function draw(){
  ...
  const row=(r,k)=>
    `<div class="row${r.grp?' grp':''}${sel===k?' sel':''}${msel.has(k)?' msel':''}" data-a="${esc(k)}" role="option" tabindex="0" aria-selected="${sel===k||msel.has(k)?'true':'false'}"${isFeat(k)?` title="${esc(featTxt(k))} \u2014 no outline, no mesh"`:''}>
  ...
  [...L.querySelectorAll('.row')].forEach(el=>el.onclick=e=>{
    const a=el.dataset.a;
    /* Ctrl/Cmd adds the row to the selection, or takes it out, and stays on this plate */
    if(e&&(e.ctrlKey||e.metaKey)){ selToggle(a); return; }
    msel.clear(); select(a);
...
function select(a,keep){
  ...
  /* picking something else starts the selection over; redrawing the same one, as a
     change of setting does, keeps whatever was Ctrl-clicked in beside it */
  if(!keep&&a!==sel) msel.clear();
  msel.delete(a);
...
function clear(){
  if(!sel) return;
  sel=null; msel.clear(); gfilter=null; $('det').hidden=true; refine();
  mark(); fit(); tgSync(); queueHash();
}
/* Ctrl/Cmd-click: add a region to the selection, or take one out. The first becomes the
   selection proper, whose card is shown; taking that one out hands the card to the one
   added most recently. */
function selToggle(a){
  if(!byAb[a]) return;
  if(!sel){ select(a); return; }
  if(a===sel){
    const nx=[...msel].pop();
    if(nx===undefined){ clear(); return; }
    select(nx,true); return;
  }
  if(msel.has(a)) msel.delete(a); else msel.add(a);
  draw(); mark(); queueHash();
}
...
function go(k){
  ...
  labels(); galMark(); anDraw(); anClose(); preload(); mcDraw();
...
/* the regions Ctrl-clicked in beside the selection, marked as it is: the outline where
   there is one, the circle round the printed name where there is not. Nothing is said
   about the ones not on this plate beyond the count, and the view is not moved for them. */
function mselShape(a,pl){
  const by=pl===undefined?regBy:regBuild(pl).by, o=by[a], rg=o&&!regUnd(o)?o:null;
  if(rg) return {rg};
  const p=pl===undefined?cur:pl, bs=isGrp(a)?[]:((LB[p]||{})[a]||[]);
  return bs.length ? {at:bs.map((b,i)=>{ const [x,y]=ptAt(p,a,i,b), R=Math.max(b[2]*NW,b[3]*NH);
    return [x*NW,y*NH,R*0.85+7,R*0.55+6]; })} : null;
}
/* the export captions name the selection; the ones beside it are named after it */
const mselCap = () => msel.size ? '   +   '+[...msel].map(a=>byAb[a].abbr).join(', ') : '';
function markMulti(){
  if(!sel||!msel.size) return;
  let h='', n=0;
  for(const a of msel){
    const q=mselShape(a); if(!q) continue; n++;
    const c=(isGrp(a)?'grp ':'')+'ms';
    h+=q.rg ? `<path class="${c}${regEst(q.rg)?' est':''}" d="${regD(q.rg)}"></path>`
      : q.at.map(([x,y,rx,ry])=>`<ellipse class="${c}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" rx="${rx.toFixed(1)}" ry="${ry.toFixed(1)}"></ellipse>`).join('');
  }
  $('om').insertAdjacentHTML('beforeend',h);
  const on=n+(mselShape(sel)?1:0), all=msel.size+1;
  vhAdd(` \u00b7 <b>${all}</b> selected, ${on===all?'all':on} on this plate \u00b7 `+
    `${esc([...msel].map(a=>byAb[a].abbr).join(', '))} beside <b>${esc(byAb[sel].abbr)}</b>`+
    ` \u00b7 <kbd>Ctrl</kbd>-click to add or remove`);
}
...
function cmpMark(){
  ...
  const by=regBuild(cmpPlate()).by;
  g.innerHTML=[sel,...msel].map(a=>{ const rg=by[a];
    return rg&&!regUnd(rg) ? `<path class="${isGrp(a)?'grp':''}${a!==sel?' ms':''}${regEst(rg)?' est':''}" d="${regD(rg)}"></path>` : '';
  }).join('');
}
...
/* largest first, so a region that lies inside another is not painted over by it; the fill
   carries the same even-odd rule the outlines do, so a hole stays a hole */
function mcRegs(pl){
...
function mcSet(on){
  mcOn=!!on; $('ckmc').checked=mcOn; $('mcw').hidden=!mcOn;
...
$('mcs').oninput=e=>{
  ...
  $('mc').style.opacity=$('mc2').style.opacity=(mcWash/100).toFixed(2);
  queueHash();
};
$('mcs').ondblclick=()=>{ $('mcs').value=45; $('mcs').dispatchEvent(new Event('input')); };
...
IW.addEventListener('click',e=>{
  ...
  const h=pickAny(f[0],f[1]);
  /* bare plate, or a face the atlas leaves unnamed; with Ctrl held that is a slip while
     adding to the selection, not a request to drop it */
  if(!h){ if(!(e.ctrlKey||e.metaKey)) clear(); }
  else if(!byAb[h.ab]) return;       /* every printed label is indexed, but guard as showTip() does */
  else if(e.ctrlKey||e.metaKey) selToggle(h.ab);   /* add it to the selection, or take it out */
  else { msel.clear(); select(h.ab); }
});
...
function exportPNG(){
  ...
  /* and the regions Ctrl-clicked in beside it, each in its own color the same way */
  for(const a of sel?[sel,...msel]:[]){
    const bs=(LB[cur]||{})[a]||[], rgS=regOut(a);
    g.strokeStyle=markC(a); g.lineWidth=1.8;
    if(rgS){
      g.beginPath();
      rgS.gs.forEach(gg=>{ gg.forEach(([x,y],i)=>i?g.lineTo(x,y):g.moveTo(x,y)); g.closePath(); });
      g.fillStyle=markF(a,'.10'); g.fill('evenodd');
      g.lineWidth=1.5; if(regEst(rgS)) g.setLineDash([6,4]);
      g.stroke(); g.setLineDash([]);
    } else if(!isGrp(a)) bs.forEach((b,i)=>{
      const [cx,cy]=ptAt(cur,a,i,b), R=Math.max(b[2]*NW,b[3]*NH);
  ...
  g.fillText(`Plate ${cur}`+(sel&&byAb[sel]?`   ·   ${byAb[sel].abbr} — ${byAb[sel].name}`+mselCap():''),0,NH+24);
...
function exportSVG(){
  ...
  /* as on the screen and the PNG: the extent where there is one, otherwise the circle.
     The selection is #highlight; each region Ctrl-clicked in beside it is a group of its
     own, named for it, so an editor can still take them apart */
  for(const a of sel?[sel,...msel]:[]){
    const bs=(LB[cur]||{})[a]||[], rgS=regOut(a);
    const id=a===sel?'id="highlight"':`class="highlight" data-abbr="${esc(a)}"`;
    if(rgS) o.push(`<g ${id} fill="${markC(a)}" fill-opacity=".10" fill-rule="evenodd"`+
      ` stroke="${markC(a)}" stroke-width="1.5"${regEst(rgS)?' stroke-dasharray="6 4"':''}>`+
      `<path d="${regD(rgS)}"/></g>`);
    else if(bs.length) o.push(`<g ${id} fill="none" stroke="${markC(a)}" stroke-width="1.8">`+
      bs.map((b,i)=>{ const [cx,cy]=ptAt(cur,a,i,b), R=Math.max(b[2]*NW,b[3]*NH);
  ...
  cap.push(svgText(0,NH+24,19,'#1b1a17',
    `Plate ${cur}`+(sel&&byAb[sel]?`   ·   ${byAb[sel].abbr} — ${byAb[sel].name}`+mselCap():''),600));
...
/* The skull silhouette and the landmark rules come along too, but neither can simply be
   turned. The stored silhouette was flattened at the atlas's angle, and the outline of a
   turned skull is not the turned outline of a skull -- so a turned view flattens the mesh
   again at the frame's angle (skSil). A landmark rule is a coronal plane, and once the
   frame tilts a plane seen along an axis it is not parallel to has no edge to draw -- so a
   turned view draws where the plane cuts the section the view is taken through (pjLMf). */
...
const pjPad=(V,up)=>Math.max(pjsk?(V.k==='dv'?(up?0.7:3.5):3.3):0,
                             pjlm?(V.k==='dv'?(up?0.8:0):3.0):0);
const pjHi=V=>V.hi+pjPad(V,1)+fvP[V.k][1];
const pjLo=V=>V.lo-pjPad(V,0)-fvP[V.k][0];
...
function fvBounds(){
  ...
  /* the landmark marks are points, and a turn can carry them past the margin the atlas's
     angle was given for them -- the ear canals, at ML +/-10.4, first. So each view also
     grows, in the same whole millimeters, to hold the marks it draws, with 0.3 mm to spare
     for the dot. The skull is not held this way: its outline runs past the axes at the
     atlas's angle too (the jaw, the nose) and is clipped to the plot as it is there. */
  if(pjlm) for(const V of Object.values(VIEWS)){
    let top=-1e9, bot=1e9;
    for(const p of pjLmPts(V)){ const c=fRot(p.ap,p.ml,p.dv)[V.k];
      if(c+0.3>top) top=c+0.3; if(c-0.3<bot) bot=c-0.3; }
    const hi=pjHi(V), lo=pjLo(V);
    fvP[V.k][1]+=hiPad(top,hi); fvP[V.k][0]+=lowPad(bot,lo);
  }
...
function pjSk(){
  ...
  const V=VIEWS[pview];
  g.innerHTML=((fvOn()?skSil(V.k):SK.sil[V.k])||[]).map(lp=>
...
function pjLM(){
  ...
  if(fvOn()){ pjLMf(V,lm,o,y1); g.innerHTML=o.join(''); return; }
  LM.forEach(([nm,off])=>{
    const a=-off, x=pjx(a);
...
/* The points a view marks, in atlas millimeters: the vault at each landmark's AP, which
   only the sagittal view draws, and the two ear canals, which both do. The heights come
   off the skull fit; without it there are no points, only the rules. */
function pjLmPts(V){
  const SK=window.__SKULL__, lm=SK&&SK.lm, o=[];
  if(!lm) return o;
  if(V.k==='dv') LM.forEach(([nm,off])=>{ const h=lm.vault[nm.toLowerCase()];
    if(h!==undefined&&nm!=='Interaural') o.push({ap:-off,ml:0,dv:h}); });
  const ia=-LMof('Interaural');
  o.push({ap:ia,ml:-lm.ear.ml,dv:lm.ear.dv},{ap:ia,ml:lm.ear.ml,dv:lm.ear.dv});
  return o;
}
/* a segment in plot units cut to the plot rectangle (Liang-Barsky), or null if none of
   it is inside */
function pjClip(a,b,yb){
  const dx=b[0]-a[0], dy=b[1]-a[1]; let t0=0, t1=1;
  for(const [p,q] of [[-dx,a[0]-PM.l],[dx,PM.l+PWv-a[0]],[-dy,a[1]-PM.t],[dy,yb-a[1]]]){
...
/* The landmarks in a turned view. A rule is a coronal plane, and a turned plane seen along
   an axis it is no longer parallel to covers the whole plot, so each is drawn where it
   cuts the section the view is taken through: the midline in the sagittal view, and in the
   top-down one the horizontal through the landmark's own point -- on the vault, or for the
   interaural line the ear-bar axis, so that rule is the axis carried across the plot
   (mid-brain, as the plate guide is, where the fit gives no height). Both pass through the
   landmark, and where the turn leaves the plane parallel to the axis dropped -- a pitch
   alone in the sagittal view, a yaw alone in the top-down one -- the line is the plane's
   edge exactly. The vault marks and the ear canals are points and turn exactly. In the
   sagittal view the ear-bar axis, a point end-on at the atlas's angle, opens under a roll
   or yaw into the short line between the two canals, and its height reference is the
   ear-bar plane's cut at the midline, tilted as the frame tilts it. Everything is turned
   by the rotation alone, as the dots are -- the translation is in the axis labels. */
function pjLMf(V,lm,o,y1){
  const R=40, pt=p=>{ const c=pjq(p); return [pjx(c.ap), pjy(V,c[V.k])]; };
  const inside=([x,y])=>x>=PM.l-1&&x<=PM.l+PWv+1&&y>=PM.t-1&&y<=y1+1;
  const seg=(p,q,cls)=>{ const s=pjClip(pt(p),pt(q),y1); if(!s) return null;
    o.push(`<line${cls?` class="${cls}"`:''} x1="${s[0].toFixed(1)}" y1="${s[1].toFixed(1)}"`+
           ` x2="${s[2].toFixed(1)}" y2="${s[3].toFixed(1)}"/>`);
    return s; };
  const dot=p=>{ const c=pt(p); if(inside(c))
    o.push(`<circle cx="${c[0].toFixed(1)}" cy="${c[1].toFixed(1)}" r="4"/>`); };
  const put=[];                               /* label boxes already placed, [x0,x1,y] */
  LM.forEach(([nm,off])=>{
    const a=-off, h=lm?lm.vault[nm.toLowerCase()]:undefined;
    /* the interaural line's own point is the ear-bar axis, not the vault above it */
    const d=!lm ? PJMID() : nm==='Interaural' ? lm.ear.dv : h===undefined ? PJMID() : h;
    const s = V.k==='dv'
      ? seg({ap:a,ml:0,dv:-R},{ap:a,ml:0,dv:R},'ref')
      : seg({ap:a,ml:-R,dv:d},{ap:a,ml:R,dv:d},'ref');
    if(!s) return;
    /* named at the rule's lower end, as the atlas's view names it at the foot of the plot.
       A tilted rule can end near the plot's right edge, where the name is set to its left;
       and tilted rules end closer together than upright ones, so a name that would overrun
       one already placed goes up a line. Widths are estimated at 7 units a character. */
    const nmL=nm.toLowerCase(), w=nmL.length*7, [x,yb]=s[1]>s[3]?[s[0],s[1]]:[s[2],s[3]];
    const end=x+4+w>PM.l+PWv, x0=end?x-4-w:x+4, x1=x0+w;
    let y=yb-6;
    while(put.some(b=>b[2]===y&&x0<b[1]+6&&x1>b[0]-6)) y-=15;
    put.push([x0,x1,y]);
    o.push(`<text x="${(end?x-4:x+4).toFixed(1)}" y="${y.toFixed(1)}"`+
           `${end?' text-anchor="end"':''}>${esc(nmL)}</text>`);
    if(V.k==='dv'&&lm&&h!==undefined&&nm!=='Interaural') dot({ap:a,ml:0,dv:h});
  });
  if(!lm) return;
  const ia=-LMof('Interaural'), e=lm.ear;
  const L={ap:ia,ml:-e.ml,dv:e.dv}, Rt={ap:ia,ml:e.ml,dv:e.dv};
  if(V.k==='dv') seg({ap:ia-R,ml:0,dv:e.dv},{ap:ia+R,ml:0,dv:e.dv},'ref');
  const pl=pt(L), pr=pt(Rt);
  if(V.k==='ml'||Math.hypot(pl[0]-pr[0],pl[1]-pr[1])>1) seg(L,Rt,'');
  dot(L); dot(Rt);
}
/* everything a change of view or of the skull toggle has to rebuild, in one place:
   ...
   silhouette and the plate guide are position-dependent and do not. The bounds go
   first: a turned view sizes itself to the landmark marks when they are on. */
function pjRefresh(){ fvBounds(); pjAxes(); pjSk(); pjLM(); pj(); tgPJ(); anPJ(); }
...
function pj(){
  ...
  const ck=V.k+(pjsk?'+':'')+(pjlm?'L':'')+(fvOn()?'F':'');
...
const pjTurn = () => !fvOn() ? '' :
  ...
  ` resectioning.`+(pjsk?` The skull outline is the fitted skull flattened again at your`+
  ` frame's angle.`:'')+(pjlm?` Each landmark rule is where its coronal plane, turned, cuts`+
  ` ${pview==='dv'?'the midline':'the horizontal through the landmark (on the vault; for the'+
  ' interaural line, the ear-bar axis)'}.`:'');
function pjNote(q){
...
function fvApply(force){
  ...
  if(force||fvOn()||fvLast){ bgd={}; fld={}; pjRefresh(); v3frame(); }
  else { pjAxes(); pjGuide(); }
  fvLast=fvOn();
  v3note();
}
...
/* The skull's outline in a turned projection. The stored silhouettes were flattened at the
   atlas's angle, and the outline of a turned skull is not the turned outline of a skull,
   so the mesh is flattened again here: every vertex turned by the frame's rotation (the
   rotation only, as the label dots are), every triangle filled into a 0.1 mm grid on the
   two plotted axes, and the edge of what is filled traced as closed loops (marching
   squares) and thinned to within 0.05 mm. A cell counts as covered within half a cell of a
   triangle, so no sliver of the 0.42 mm decimation leaves a pinhole, and holes under 5 mm²
   are dropped, as the stored silhouettes drop them. Checked against those at no rotation:
   every stored point lies within 0.27 mm of the traced outline (median 0.14); the traced
   one also follows a notch in the skull base (AP -4 to -6, DV -13 to -16) that the stored
   one bridges. Cached per view and rotation, so a change of anything else does not redo it. */
let SKF={};
function skSil(k){
  const M=skullMesh(); if(!M) return [];
  const key=k+FRM.join(',');
  if(SKF[key]) return SKF[key];
  if(Object.keys(SKF).length>=16) SKF={};     /* typing an angle is one entry a keystroke */
  const h=0.1, e=0.5, {pos,idx,nf}=M, n=pos.length/3, X=new Float32Array(n), Y=new Float32Array(n);
  let x0=Infinity, x1=-Infinity, y0=Infinity, y1=-Infinity;
  for(let i=0;i<n;i++){ const c=fRot(pos[i*3],pos[i*3+1],pos[i*3+2]), x=X[i]=c.ap/h, y=Y[i]=c[k]/h;
    if(x<x0) x0=x; if(x>x1) x1=x; if(y<y0) y0=y; if(y>y1) y1=y; }
  /* grid units are cells, with an empty border of two so every loop closes */
  const ox=Math.floor(x0)-2, oy=Math.floor(y0)-2;
  const W=Math.ceil(x1)-ox+3, H=Math.ceil(y1)-oy+3, g=new Uint8Array(W*H);
  for(let f=0;f<nf;f++){
    const a=idx[f*3], b=idx[f*3+1], c=idx[f*3+2];
    const ax=X[a]-ox, ay=Y[a]-oy; let bx=X[b]-ox, by=Y[b]-oy, cx=X[c]-ox, cy=Y[c]-oy;
    const ar=(bx-ax)*(cy-ay)-(by-ay)*(cx-ax);
    if(ar===0) continue;                          /* edge-on: its neighbours cover it */
    if(ar<0){ let t=bx; bx=cx; cx=t; t=by; by=cy; cy=t; }
    let ux=bx-ax, uy=by-ay, l=Math.hypot(ux,uy); const p0=ux/l, q0=uy/l;
    ux=cx-bx; uy=cy-by; l=Math.hypot(ux,uy); const p1=ux/l, q1=uy/l;
    ux=ax-cx; uy=ay-cy; l=Math.hypot(ux,uy); const p2=ux/l, q2=uy/l;
    const i0=Math.max(0,Math.ceil(Math.min(ax,bx,cx)-e)), i1=Math.min(W-1,Math.floor(Math.max(ax,bx,cx)+e));
    const j0=Math.max(0,Math.ceil(Math.min(ay,by,cy)-e)), j1=Math.min(H-1,Math.floor(Math.max(ay,by,cy)+e));
    for(let j=j0;j<=j1;j++) for(let i=i0;i<=i1;i++){
      if(p0*(j-ay)-q0*(i-ax)<-e||p1*(j-by)-q1*(i-bx)<-e||p2*(j-cy)-q2*(i-cx)<-e) continue;
      g[j*W+i]=1;
    }
  }
  /* marching squares on the cell centers. Edge crossings are numbered by cell and side
     (even: the horizontal edge above a center, odd: the vertical one to its right), and
     each crossing is met by exactly two segments, so the loops chain without a search.
     The two saddles are read as joined, which keeps a one-cell neck of bone in one piece. */
  const T=[[],[3,0],[0,1],[3,1],[1,2],[3,2,0,1],[0,2],[3,2],[3,2],[0,2],[3,0,1,2],[1,2],[3,1],[0,1],[3,0],[]];
  const NE=W*H*2, adj=new Int32Array(NE*2).fill(-1);
  const eid=(i,j,s)=> s===0 ? (j*W+i)*2 : s===2 ? ((j+1)*W+i)*2 : s===3 ? (j*W+i)*2+1 : (j*W+i+1)*2+1;
  const link=(p,q)=>{ adj[p*2+(adj[p*2]<0?0:1)]=q; adj[q*2+(adj[q*2]<0?0:1)]=p; };
  for(let j=0;j<H-1;j++) for(let i=0;i<W-1;i++){
    const t=T[g[j*W+i]|g[j*W+i+1]<<1|g[(j+1)*W+i+1]<<2|g[(j+1)*W+i]<<3];
    for(let s=0;s<t.length;s+=2) link(eid(i,j,t[s]),eid(i,j,t[s+1]));
  }
  const at=p=>{ const c=p>>1, i=c%W, j=(c-i)/W;
    return p&1 ? [(ox+i)*h,(oy+j+.5)*h] : [(ox+i+.5)*h,(oy+j)*h]; };
  const seen=new Uint8Array(NE), out=[];
  for(let s=0;s<NE;s++){
    if(adj[s*2]<0||seen[s]) continue;
    const L=[]; let p=s, prev=-1;
    while(!seen[p]){ seen[p]=1; L.push(at(p)); const nx=adj[p*2]===prev?adj[p*2+1]:adj[p*2]; prev=p; p=nx; }
    let A=0; for(let i=0;i<L.length;i++){ const u=L[i], v=L[(i+1)%L.length]; A+=u[0]*v[1]-v[0]*u[1]; }
    if(Math.abs(A)/2>=5) out.push(skThin(L,0.05));
  }
  return SKF[key]=out;
}
/* Douglas-Peucker on a closed loop, split at the point farthest from its first */
function skThin(L,tol){
  const n=L.length, keep=new Uint8Array(n); let far=0, fd=-1;
  for(let i=1;i<n;i++){ const d=(L[i][0]-L[0][0])**2+(L[i][1]-L[0][1])**2; if(d>fd){ fd=d; far=i; } }
  keep[0]=keep[far]=1;
  const st=[[0,far],[far,n]];
  while(st.length){
    const [a,b]=st.pop(), P=L[a], Q=L[b%n], dx=Q[0]-P[0], dy=Q[1]-P[1], l=Math.hypot(dx,dy)||1;
    let m=-1, md=tol;
    for(let i=a+1;i<b;i++){ const d=Math.abs(dy*(L[i][0]-P[0])-dx*(L[i][1]-P[1]))/l; if(d>md){ md=d; m=i; } }
...
function writeHash(){
  ...
  /* the regions Ctrl-clicked in beside it, each encoded, so a comma is only ever the separator */
  if(sel&&msel.size) h+='&ms='+[...msel].map(encodeURIComponent).join(',');
...
function readHash(){
  ...
  mcOn = v.includes('C');
  $('ckmc').checked=mcOn; $('mcw').hidden=!mcOn;
  $('mcs').value=mcWash; $('mcl').textContent=mcWash+'%';
  ...
  if(a&&byAb[a]){
    select(a);
    msel.clear();
    for(let x of (par.ms||'').split(',')){ try{ x=decodeURIComponent(x); }catch(_){ continue; }
      if(x&&byAb[x]&&x!==a) msel.add(x); }
    if(msel.size){ draw(); mark(); }
  } else clear();
...
window.__gae={toFrame,fromFrame,writeHash,readHash,tgSolve,tgPath,tgFootprint,plan:()=>tgPlan,
  ...
  state:()=>({cur,sel,msel:[...msel],zoom,tab,smode,psrc,tgProbe,tgFoot,cmpOn,anShow,maxed,targSide,tgTilt,tgRoll,tgYaw,tgPlate,tgOff,fview,fvOn:fvOn(),v3two,v3ed,v3lock,mcOn,mcWash,foldOn,vpanOn,advOn,infOn})};
