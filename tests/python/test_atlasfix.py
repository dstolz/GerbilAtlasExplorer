...
def test_the_page_reads_the_published_map_the_way_the_tool_reads_the_cut():
    """The page GitHub Pages serves has no Python, so `Pick` there reads data/facemaps/
    instead of cutting the plate. That the committed maps *are* the cut is
    `build_facemaps.py --check`, which regenerates all 62 and compares the bytes; CI
    runs it beside this file, so it is not repeated here. What is checked here is the
    other half, which no rebuild can tell you: that reading a face id out of the
    published raster and looking it up in the sidecar gives the answer `probe` gives
    from the live cut -- the same face, the same area, the same seeding labels."""
    import gzip
    S = session()
    P = S.plate(PLATE)
